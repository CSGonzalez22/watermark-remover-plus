...
Any tool that claims to "remove the SynthID watermark" from text with a
character filter is selling you nothing. Layer A and C are real work with a
verifiable result; Layer B is a rewrite with an honest asterisk.
...
single stdlib CLI and a research file that shows its work.
